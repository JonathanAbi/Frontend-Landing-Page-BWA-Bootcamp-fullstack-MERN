...
export async function getServerSideProps(context) {
  //errorHandler di client-side tidak bisa dipanggil langsung untuk menangani error pada getServerSideProps.
  try {
    ...
    // Cek jika pesan error adalah "jwt expired"
    if (error.response && error.response.data && error.response.data.msg === "jwt expired") {
      // Redirect ke halaman sign-in jika token expired
      return {
        redirect: {
          destination: '/signin',
          permanent: false,
    ...
    // Jika error lain terjadi, bisa juga diarahkan ke halaman error
    return {
      props: { data: [] }, // Atau sesuaikan penanganan sesuai kebutuhan
    };
  }
}